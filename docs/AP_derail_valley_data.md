...
### Station licenses
A new mechanic added to the randomizer are station licenses. There exists one license for each station (for a total of 20). You need the corresponding license in order to take any job and to check the demonstrator locomotives locations (see below). There are 20 station licenses rated progression.
